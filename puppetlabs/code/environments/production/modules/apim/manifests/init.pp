...
class apim inherits apim::params {
  ...
  */
  ...
  # Copia dei file custom
  $common_file_list.each | String $file | {
  ...
  file { "/usr/wso2/log-compress.sh":
     owner  => $user,
     group  => $user_group,
     mode   => '0755',
     source => "puppet:///modules/${module_name}/system/usr/wso2/log-compress.sh",
   }
   
   #rimozione dei file
   $remove_file_list.each | String $file | {
    file { "${install_path}/${file}":
      ensure  => absent,
      owner  => $user,
      group  => $user_group,
